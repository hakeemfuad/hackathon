...
The combined build also includes the undergarments and live-update changes from the parallel Qloset task. An opaque sports top and fitted shorts replace covered skin faces for display while leaving the complete measurement surface intact. The garments follow the subdivided mesh when profiles change. Choice fields update immediately; numeric fields debounce for 250 ms without resetting the input caret. Browser verification confirmed automatic weight updates, changed circumference results, front/back coverage, and no console warnings or errors.
